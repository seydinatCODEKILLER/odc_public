...
function paginateQuery(string $baseSql, array $params = [], int $currentPage = 1, int $perPage = 10): array
{
    ...
    // Compter le nombre total d'éléments (version corrigée)
...
function generatePageLink(array $queryParams, int $page): string
{
    ...
    $existingParams = $_GET;
    unset($existingParams['p']);
    $mergedParams = array_merge($existingParams, $queryParams, ['p' => $page]);

    // Retourne l'URL complète
    return $currentPath . (!empty($mergedParams) ? '?' . http_build_query($mergedParams) : '');
}
